...
import { PayDeclinedDesktopComponent } from '../../modals/pay-declined/desktop/pay-declined-desktop.component';

@Component({
  templateUrl: 'new-card-payment-desktop.component.html',
  styleUrls: ['new-card-payment-desktop.component.scss'],
  host: { 'app.new-card-payment-desktop': 'true' }
})
export class NewCardPaymentDesktopComponent extends ViewComponent {
  ...
  payDecline() {
    this.dialog.dismiss().then(() => {
      if (this.allFieldsCompleted) {
        this.dialog.showWithData({
          component: PayDeclinedDesktopComponent,
          cssClass: ['modal-custom', 'modal-custom--in-center-90']
        });
      }
    });
